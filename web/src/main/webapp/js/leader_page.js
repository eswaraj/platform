...
jQuery(document).ready(function($){
	...
	$('#mla-bio-data-modal-innerdiv .close-btn').on('click', function(){
		$('#mla-bio-data-modal').fadeOut(1);
		$('.main_content_page').fadeTo( "slow", 1 );
	});
...
window.onload = function() {
	...
	$('#mla_bio_data_link').on('click', function(){
		$('#mla-bio-data-modal').fadeIn(1);
		$('.main_content_page').fadeTo( "slow", 0.33 );
		$("html, body").animate({ scrollTop: 0 }, "slow");
	});
	...
	$("#mla-bio-data-modal").click(function(e) {
		if($(e.target).is('#mla-bio-data-modal-innerdiv')){
			e.preventDefault();
			return;
		}
		$('.main_content_page').fadeTo( "slow", 1 );
