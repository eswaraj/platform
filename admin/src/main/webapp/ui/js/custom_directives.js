    ...
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            scope.$on('addRoot', function(event, data) {
                if(data.id === attrs.id) {
                    element.jstree(true).create_node('#', {
                        id : data.child.id,
                        text : scope.displayFunction({obj : data.child}),
                        li_attr : data.child
                    });
                }
            });
            ...
            scope.$on('updateNode', function(event, data) {
                if(data.id === attrs.id) {
                    element.jstree('set_text',data.child.id, scope.displayFunction({obj : data.child}));
            ...
            request.success(function(data) {
                if(data instanceof Array) {
                    for(var i=0; i< data.length;i++){
                        var new_node = {
                            'text': scope.displayFunction({obj : data[i]}),
                            'id': data[i].id,
                            'li_attr': ""
                        };
                    ...
                    var new_node = {
                        'text': scope.displayFunction({obj : data}),
                        'id': data.id,
                        'li_attr': ""
                    };
