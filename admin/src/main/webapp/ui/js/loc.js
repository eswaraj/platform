	...
	$('#js_tree').jstree({ 'core' : {
		...
		$.ajax({
			...
			success: function(data){
				$("#files tbody").html("");
				for(var i=0;i < data.length; i++){
					$("#files tbody").append("<tr><td>"+data[i].originalFileName+"</td><td><a  href='#' onclick='update_map("+data[i].fileNameAndPath+");'>ShowOnMap</a></td><td><a href='#' onclick='set_current("+data[i].fileNameAndPath+");'>ChooseAsCurrent</a></td></tr>");
				}
			}
		});
