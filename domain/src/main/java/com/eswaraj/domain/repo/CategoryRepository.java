...
public interface CategoryRepository extends GraphRepository<Category>{
    ...
    @Query("start childCategory=node({0}) match (category)<-[:BELONGS_TO]-(childCategory) where category.__type__ = 'com.eswaraj.domain.nodes.Category' or category.__type__ = 'Category' return category")
    public Category getParentCategory(Category category);

    @Query("start complaint=node({0}) match (complaint)-[:BELONGS_TO]->(category) where category.__type__ = 'com.eswaraj.domain.nodes.Category' or category.__type__ = 'Category' return category")
