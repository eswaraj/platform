...
public interface UserRepository extends GraphRepository<User>{
    ...
    @Query("start person=node({0}) match (user)-[:ATTACHED_TO]->(person) where user.__type__ = 'User' return user")
    public User getUserByPerson(Person person);

    @Query("start device=node({0}) match (user)-[:USER_DEVICE]->(device) where user.__type__ = 'User' return user")
    ...
    @Query("start facebookAccount=node({0}) match (user)<-[:OF_USER]-(facebookAccount) where user.__type__ = 'User' return user")
